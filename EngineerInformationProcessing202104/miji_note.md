...
⚡️[PART 13 제품소프트웨어 패키징 및 용어](#part-13-제품소프트웨어-패키징-및-용어)
...
11. #### 컴퓨터 프로그래밍의 패러다임 중 하나로 컴퓨터 프로그램을 명령어의 목록으로 보는 시각에서 벗어나 여러 개의 독립된 단위, 즉 객체들의 모임으로 파악하고자 하는 것으로, 각각의 객체는 메시지를 주고받고, 데이터를 처리할 수 있는 프로그래밍 기법은?
...
| 테스팅은 개발 초기에 시작해야 함 | 애플리케이션의 개발 단계에 테스트를 기획하고 SDLC의 각 단계에 맞춰 전략적으로 접근하는 것을 고려 |
...
246. #### 사원(EMPLOYEE) 테이블에서 이름(EMPNAME)의 가운데가 '상'인 사원의 이름(EMPNAME)과 급여(SALARY)를 검색하는 SELECT 문을 작성
...
## PART 13 제품소프트웨어 패키징 및 용어

463. #### OSI 7계층에서 - 단말 간 시스템끼리 데이터를 전송하기 위한 최선의 통신경로 선택을 제공하는 계층은 () 계층이다.
...
| 계층                                    | 설명                                                         | 주요 프로토콜                                 | 단위    |
| --------------------------------------- | ------------------------------------------------------------ | --------------------------------------------- | ------- |
| 응용 계층<br />(Application Layer)      | 사용자와 네트워크 간의 응용 서비스 연결, 데이터 생성         | HTTP, TELNET, DHCP, DNS, FTP, SSH, SMTP, SNMP | Data    |
| 표현 계층<br />(Presentation Layer)     | 데이터의 형식 설정과 부호 교환,  암호화, 해독                | MIME, TLS, SSL, JPEG, MPEG, SMB, AFP          | Data    |
| 세션 계층<br />(Session Layer)          | 응용 프로세스 간의 연결 접속 및 동기 제어                    | SSH, TLS, RPC                                 | Data    |
| 전송 계층<br />(Transport Layer)        | - 프로세스 간 논리적 통신 서비스 제공<br />- 패킷들의 전송 유효 확인, 실패한 패킷은 재전송하여 신뢰성 있는 통신 보장<br />- 오류검출과 복구, 흐름제어 | TCP(3-Way Handshaking), UDP, SCTP, RTP        | Segment |
| 네트워크 계층<br />(Network Layer)      | 단말 간 시스템끼리 Data를 전송하기 위한 최선의 통신경로 선택을 제공 | IP, ARP, ICMP, IGMP, IPsec                    | Packet  |
| 데이터 링크 계층<br />(Data Link Layer) | - 인접 시스템 간의 데이터 전송, 전송 오류 제어(Frame)<br />- 오류 검출, 재전송, 흐름 제어 | Ethernet, ATM, PPP                            | Frame   |
| 물리 계층<br />(Physical Layer)         | 통신회선으로 데이터를 나타내는 0과 1 비트의 정보를 회선에 내보내기 위한 전기적 변환이나 기계적 작업을 담당 | RS-485, RS-232, X25/21                        | Bits    |
...
464. #### 네트워크 통신 프로토콜의 역할 중 송신 측에서 전송한 데이터의 각 비트를 수신 측에서 정확하게 수신할 수 있도록 하는 기법을 무엇이라고 하는가?
...
동기화
  ...
  ##### 해설
    ...
    비동기식 전송: 송수신기가 별도의 독립적인 클록을 사용하면서도 데이터를 송수신
...
```
IPv6는 IPv4 주소 부족 문제를 해결하기 위해 개발 되었으며 16비트씩 8부분, 총 128비트로 구성되어 있는 논리적인 주소체계이다.
```



467. #### 컴퓨터에서 발생하는 디지털 신호를 아날로그 신호로 바꿔주는 변조방식은?
...
<img src ="./img/468.jpeg">



<details> <summary>답</summary> <div markdown="1">  

<img src = "./img/468-1.jpeg">



</div> </details>

469. 송수신의 동기화 기법 중 아래에서 설명하는 기법은 () 전송기법이다,
...
470. #### 아날로그-디지털 부호화 방식인 송신측 PCM(Pulse Code Modulation) 과정 중 (A)에 들어갈 단어는?
...
양자화
  ...
  ##### 해설(추가 개념)
    ...
    표본화: 연속적으로 변화하는 값을 일정한 시간적 간격으로 나누어 표본을 추출
    양자화: 연속적인 아날로그 값을 이산적인 디지털 값으로 바꾸는 A/D 변환
...
471. #### 고속의 전송로 하나에 데이터 신호 여러 개를 중복시켜 고속 신호 하나를 만들어 전송하는 기법은?
...
<img src = "./img/471.jpeg">

</div> </details>



472. #### 통신 프로토콜의 3요소 중 다음 기능을 하는 기본 요소를 적으시오

```
- 데이터의 구조나 형태(포맷)를 정의
- 16비트 정보 송신 시 앞의 8bit는 주소, 다음 8bit는 데이터로 정의
```
...
```
3요소
1. 구문 : 데이터의 구조나 형태를 정의
2. 타이밍: 어떤 데이터를 보낼 것인지, 얼마나 빨리 보낼 것인지를 결정
3. 의미: 데이터의 각 부분이 무엇을 뜻하는지 알 수 있게 미리 정해 둔 규칙
```
